...
::: callout-note
The shape file for Nepal can be obtained from Government of [Government of Nepal geoportalal geoportal](https://nationalgeoportal.gov.np/#/map) and availability of human resource data from [Open data Nepal](https://opendatanepal.com/dataset?category=Health&page=4).
:::
...
ggplot(data=dat) +geom_sf (aes(fill=Fulfilled_percent), color="black", size=0.2) +scale_fill_viridis_c(option = "viridis", trans="sqrt")+ geom_text(data=points, aes(x=X, y=Y, label= paste(DISTRICT)), color="darkblue", size = 2.5) + ggtitle("Fullfilled percentage of sanctioned post of Medical doctors in Nepal 2072/73") +theme_void()
